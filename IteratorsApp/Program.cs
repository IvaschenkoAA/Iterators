...
class Program
{
    private static void Main()
    {
        ...
        IEnumerable<string> lines1 = ReadFileUsingReturn(path);
        IEnumerable<string> lines2 = ReadFileUsingLinq(path);
        IEnumerable<string> lines3 = ReadFileUsingYieldReturn(path);

        PrintFileContents($"Вывод строк из файла {path} через return:", lines1);
        PrintFileContents($"Вывод строк из файла {path} через LINQ:", lines2);
        PrintFileContents($"Вывод строк из файла {path} через yield return:", lines3);
